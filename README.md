...
Clones `JasonMore/ai-skills` and runs its installer so personal Copilot skills persist across Codespaces. Personal AI skills always install last, so they win any same-name skill conflicts with other installers (e.g. `install-agent-skills`, `install-caveman-skills`).
