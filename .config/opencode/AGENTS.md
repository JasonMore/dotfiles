...
- **Clear writing** - use short, direct prose
...
### Conciseness
Be concise and clear. Use ASD-STE100 Simplified Technical English for prose.
...
2. Switch to analysis mode. Write out what was tried, what happened, and possible root causes
3. Return to implementation with a clear new approach
...
- Infer types where possible. Do not add explicit types for `map()`, `filter()`, `find()` callbacks
...
**Default: direct work.** Use the main session for small, clear changes.

### Spawning Rules
- **Only spawn subtasks when user requests it or the task is broad, risky, unfamiliar, or spans several files**
- **Use one subtask by default**
- **NEVER auto-spawn** because a previous subtask returned empty or appeared to stop. Stop and wait for user input instead
- Do not spawn more than one subtask for the same coding task unless the user asks for parallel review or the work has separate parts.
...
### Handoff Commands
- `/subtask-complete [topic]` - write handoff file that summarizes what was done
- `/subtask-resume` - load all pending handoff files to sync orchestrator state

### Verification Before Completion
Before claiming a task is "complete":
1. Run the smallest concrete check that covers the change.
2. State what was verified and the result.
3. If remaining work is found, continue. Do not claim partial work is complete.
